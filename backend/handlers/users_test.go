...
import (
	...
	// Driver "sqlite" = modernc.org/sqlite (pure Go, tanpa CGO) — sama seperti yang
	// dipakai database.Init() supaya perilaku DB di test mendekati produksi.
...
func setupUserTestDB(t *testing.T) {
	...
	db, err := gorm.Open(
		sqlite.New(sqlite.Config{DriverName: "sqlite", DSN: dsn}),
